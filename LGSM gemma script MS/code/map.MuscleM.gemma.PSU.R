...
# SCRIPT PARAMETERS
# -----------------
analysis <-Muscles.BMC
which.analysis <- c("TA","EDL","Gastroc","Soleus","appBMC","BT")
choose.samples <- "ALL"
use.kinship    <- TRUE
...
# Convert the sex, time of day, and study columns to binary values. Sex variable is needed to run GEMMA
...
phenotypes<-c("TA","EDL","Gastroc","Soleus","appBMC","BT")
...
gwscan.gemma.LOD$TA$log10p<- log10p2pval(gwscan.gemma.LOD$TA$log10p)
gwscan.gemma.LOD$EDL$log10p<- log10p2pval(gwscan.gemma.LOD$EDL$log10p)
gwscan.gemma.LOD$Gastroc$log10p<- log10p2pval(gwscan.gemma.LOD$Gastroc$log10p)
gwscan.gemma.LOD$Soleus$log10p<- log10p2pval(gwscan.gemma.LOD$Soleus$log10p)
#
gwscan.gemma.LOD$appBMC$log10p<- log10p2pval(gwscan.gemma.LOD$appBMC$log10p)
gwscan.gemma.LOD$BT$log10p<- log10p2pval(gwscan.gemma.LOD$BT$log10p)

# Convert pvalue to LOD scores.  
gwscan.gemma.LOD$TA$log10p<- pval2lod(gwscan.gemma.LOD$TA$log10p)
gwscan.gemma.LOD$EDL$log10p<- pval2lod(gwscan.gemma.LOD$EDL$log10p)
gwscan.gemma.LOD$Gastroc$log10p<- pval2lod(gwscan.gemma.LOD$Gastroc$log10p)
gwscan.gemma.LOD$Soleus$log10p<- pval2lod(gwscan.gemma.LOD$Soleus$log10p)
#
gwscan.gemma.LOD$appBMC$log10p<- pval2lod(gwscan.gemma.LOD$appBMC$log10p)
gwscan.gemma.LOD$BT$log10p<- pval2lod(gwscan.gemma.LOD$BT$log10p)

# change the name of the columns
names(gwscan.gemma.LOD$TA$log10p)<-c("chr","pos","lod")
names(gwscan.gemma.LOD$EDL$log10p)<-c("chr","pos","lod")
names(gwscan.gemma.LOD$Gastroc$log10p)<-c("chr","pos","lod")
names(gwscan.gemma.LOD$Soleus$log10p)<-c("chr","pos","lod")
#
names(gwscan.gemma.LOD$appBMC$log10p)<-c("chr","pos","lod")
names(gwscan.gemma.LOD$BT$log10p)<-c("chr","pos","lod")

# Save the data frame to an RData file.
save(gwscan.gemma.LOD,file="gemma.lod.RData")
