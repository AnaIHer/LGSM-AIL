...
run.gemma <- function (phenotype, covariates, pheno, geno, map,
  ...
  for (chr in chromosomes) {
    ...
    if (compute.kinship.matrix) {
      ...
      K <- center.columns(geno[,markers]) 
    # allocate 0 (no relationship) to missing values on the k matrix
	# this is ok only because we have a small number of missing, otherwise 
	# imputation is required
      K[is.na(K)] <-0 
      K <- tcrossprod(K) / length(markers)
      K <- K + diag(delta,n)
    }
